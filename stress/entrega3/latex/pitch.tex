...
\cue{Habla Ana Mar\'ia. Presenta el contexto del proyecto AGI--TAXI y por qu\'e era necesario medir su capacidad.}

\noindent Buenos d\'ias. Somos el equipo del Seminario de Voz IP, y hoy queremos contarles c\'omo aplicamos pruebas de esfuerzo sobre un sistema Asterisk real --- el proyecto \textbf{AGI--TAXI} que construimos en el mismo seminario.
...
\medskip
Ante esta pregunta, decidimos aplicar tres de las cinco t\'ecnicas de \textit{stress testing} de telefon\'ia IP que documentamos en la Entrega 1: \textbf{Load Testing} para caracterizar la zona segura, \textbf{Stress Testing} para buscar el techo, y \textbf{Spike Testing} para evaluar la resiliencia ante picos. Nuestro compa\~nero les va a contar c\'omo las implementamos.
...
\cue{Habla Juan Esteban. Lider\'o la parte t\'ecnica de configuraci\'on y orquestaci\'on de las pruebas.}
...
Para no contaminar el PBX productivo, todas las adiciones se introdujeron mediante directivas \texttt{\#include}, completamente reversibles:
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item Un \textbf{endpoint PJSIP dedicado} (\texttt{6000}, con autenticaci\'on \textit{digest}) en \texttt{pjsip-stress.conf}.
  \item Un \textbf{contexto de dialplan nuevo} (\texttt{from-stress}) con dos extensiones pareadas:
  ...
  \end{itemize}
\end{itemize}
Esto permiti\'o aislar el costo del plano de negocio frente al puro costo de la pila SIP.
...
\medskip
\textbf{Trazabilidad.} Todos los scripts, configuraciones y CSVs de resultados quedaron versionados en una rama dedicada del repositorio p\'ublico del proyecto, junto con un README que documenta paso a paso c\'omo reproducir cada prueba.
...
\cue{Habla V\'ictor. Presenta los n\'umeros, los hallazgos no obvios y las conclusiones.}

\noindent Gracias. Los n\'umeros consolidados son contundentes:
...
En resumen: aplicamos tres de las cinco t\'ecnicas de \textit{stress testing} sobre el sistema AGI--TAXI, procesamos m\'as de 105 mil llamadas con 100\,\% de \'exito, identificamos cinco hallazgos accionables y dejamos toda la evidencia versionada y reproducible en GitHub. El informe final consolida los tres entregables del seminario en un \'unico documento, listo para ser auditado.
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Material visual durante el video.} Mostrar el PDF del informe final (\texttt{informe\_final.pdf}) en pantalla mientras se habla. Pausar en:
  \begin{itemize}[leftmargin=*]
    \item Fig.~1 (arquitectura del montaje) durante PARTE 1.
    \item Listing del \texttt{pjsip-stress.conf} y la Tabla de escalones durante PARTE 2.
    \item Tabla resumen consolidada y las tres figuras de CPU (Load, Stress, Spike) durante PARTE 3.
  \end{itemize}
  \item \textbf{Demo en vivo opcional ($\approx$ 1 min).} Mostrar la pantalla final de SIPp en el último escalón del Stress Test, donde se ve: 18\,000 llamadas exitosas, 0 fallidas, 0 retransmisiones.
