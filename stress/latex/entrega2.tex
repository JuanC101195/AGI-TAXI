...
\begin{figure}[!h]
\centerline{\includegraphics[width=0.95\columnwidth]{cpu_vs_load.png}}
\caption{CPU (\textit{usr+sys+iow}) de la VM AGI--TAXI durante el Load Test del escenario AGI. Cada banda vertical sombreada corresponde a uno de los siete escalones de la Tabla~\ref{tab:escalones}.}
\label{fig:cpu}
\end{figure}
...
A lo largo de los siete escalones planificados, la VM AGI--TAXI mantuvo
el consumo de CPU por debajo del umbral cr\'itico del 80\,\% en todos
los casos. El pico m\'aximo observado, 10{,}5\,\% (\textit{usr+sys+iow})
correspondi\'o al escal\'on superior (300 concurrentes, 40 cps) bajo el
escenario AGI. La tasa de llamadas exitosas se mantuvo en 100\,\% para
los 14 escalones combinados de los dos escenarios (Echo y AGI), sin
retransmisiones ni timeouts SIP. Por consiguiente, dentro del rango
ensayado el sistema opera con holgura significativa: la \textit{capacidad
sostenida demostrada} de la VM corresponde, al menos, a 300 llamadas
concurrentes con 40 cps de tasa de llegada, observ\'andose a\'un
m\'argenes amplios para escalar.
...
\textbf{(i) Escenario sin RTP.} El escenario \texttt{uac-echo-no-rtp.xml}
anuncia codec G.711 en el SDP pero no env\'ia flujo RTP real. En
...
suficiente para evaluar la pila SIP, pero deja por fuera el costo de la
mezcla RTP en Asterisk. Una segunda iteraci\'on del trabajo deber\'ia
...
durante la rampa. Esto NO indica que el sistema no haya manejado las
300 llamadas objetivo: lo que ocurre es que cada llamada Echo/AGI sin
RTP completa el flujo INVITE--200--ACK--BYE en milisegundos, por lo que
con un muestreo a 1\,Hz casi nunca se captura el instante exacto en que
...
ausencia de retransmisiones.

\textbf{(iii) Hold-tests inviables sobre loopback.} Para forzar la
acumulaci\'on de canales se realizaron dos pruebas auxiliares
manteniendo cada llamada activa durante 30\,s y 60\,s respectivamente
mediante la bandera \texttt{-d}. Ambas terminaron con el 100\,\% de
las llamadas en estado de \textit{timeout} de retransmisi\'on UDP
sobre la interfaz \texttt{lo}, sin que Asterisk respondiera siquiera
\texttt{100 Trying} a las INVITE. El comportamiento es reproducible y
parece relacionado con la combinaci\'on de autenticaci\'on digest y
\texttt{pause} prolongado en el escenario; queda como elemento de
investigaci\'on para un trabajo posterior. Las trazas
\texttt{sipp-hold-error.log} y \texttt{sipp-hold2-error.log} se
incluyen como evidencia.

\subsection{Escalado posterior}

El procedimiento queda parametrizado para que cualquier ejecuci\'on en
un host m\'as potente o con \textit{sizing} reducido pueda extender los
escalones m\'as all\'a de 300 simplemente ajustando el arreglo
\texttt{LEVELS} de \texttt{run-load.sh}. Para encontrar el verdadero
punto de saturaci\'on en la VM actual, las dos extensiones razonables
son: subir la tasa de llegada por encima de 100\,cps en escal\'on
\'unico, y reducir los vCPUs asignados a la VM en VirtualBox para
estresar artificialmente el plano de cómputo.
